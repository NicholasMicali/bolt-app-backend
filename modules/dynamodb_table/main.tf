...
# Core table when autoscaling disabled
...
# Provisioned with autoscaling (no GSI ignore)
resource "aws_dynamodb_table" "autoscaled" {
  ...
  # (same block as above for attributes, indexes, replicas...)
  lifecycle {
    ignore_changes = [read_capacity, write_capacity]
  }
}

# Provisioned with autoscaling and ignoring GSI changes
resource "aws_dynamodb_table" "autoscaled_gsi_ignore" {
  count = var.create_table && var.autoscaling_enabled && var.ignore_changes_global_secondary_index ? 1 : 0
  # (same block as autoscaled)
  lifecycle {
